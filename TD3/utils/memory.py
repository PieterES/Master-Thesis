...
class ReplayBuffer(object):
    def __init__(
        ...
        self.cost = np.zeros((self.max_size, 1))
    ...
    def add(
        self, state, action, next_state, reward, cost, done, hiddens, next_hiddens
    ):
    ...
    def sample(self, batch_size=100):
        ...
        cost = torch.FloatTensor(
            self.cost[ind][:, None, :]).to(self.device)
        d = torch.FloatTensor(
            self.not_done[ind][:, None, :]).to(self.device)

        return s, a, ns, r, cost, d, hidden, next_hidden
    ...
    def _ff_sampling(self, ind):
        ...
        cost = torch.FloatTensor(self.cost[ind]).to(self.device)
