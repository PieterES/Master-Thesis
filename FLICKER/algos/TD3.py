...
class Actor(nn.Module):
    def __init__(
        self, state_dim, action_dim, hidden_dim, max_action, is_recurrent=LSTM
    ):
    ...
    def forward(self, state, hidden):
        ...
        if LSTM:
            ...
            a, h = self.l2(a, hidden)
        else:
            a, h = F.relu(self.l2(a)), None
...
class Critic(nn.Module):
    def __init__(
        self, state_dim, action_dim, hidden_dim, is_recurrent=LSTM
    ):
    ...
    def forward(self, state, action, hidden1, hidden2):
        ...
        if LSTM:
            self.l2.flatten_parameters()
            self.l6.flatten_parameters()
            q1, hidden1 = self.l2(q1, hidden1)
            q2, hidden2 = self.l6(q2, hidden2)
        else:
            q1, hidden1 = F.relu(self.l2(q1)), None
            q2, hidden2 = F.relu(self.l6(q2)), None
...
class TD3(object):
    ...
    def get_initial_states(self):
        ...
        if LSTM:
            h_0 = torch.zeros((
                self.actor.l2.num_layers,
                1,
                self.actor.l2.hidden_size),
                dtype=torch.float)
    ...
    def train(self, replay_buffer, batch_size=100):
        ...
        state, action, next_state, reward, cost, not_done, hidden, next_hidden = \
